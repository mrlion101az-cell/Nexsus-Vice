...
public class ViceEffectManager {
    ...
    public ViceEffectManager(ViceConfig config, ViceDataManager viceData) {
        this.config = config;
    ...
    private void triggerVomit(Player player) {
        ...
        player.getWorld().spawnParticle(Particle.DUST, loc, 16, 0.3, 0.2, 0.3,
                new Particle.DustOptions(Color.fromRGB(107, 87, 40), 1.2f));
        player.getWorld().playSound(loc, Sound.ENTITY_PLAYER_BURP, 1.0f, 0.6f);
